...
sqlite_file_path = './S&P 500.sqlite' #default path to the sqlite database
table_name = 'Stocks'
...
geolocator = Nominatim(user_agent="hq_locator") #geolocator object for getting geographic data about the company
...
class Company :
    ...
    def __init__(self, symbol, sql_path=sqlite_file_path, table=table_name) :
        ...
        for input_var in (symbol, sql_path, table): #make sure inputs are of the correct type (str)
            if not isinstance(input_var, str):
                raise TypeError(f"Input must be of type 'str', but input of type {type(input_var)} encountered.")
            
        self.symbol = symbol.upper()
        self.conn = sqlite3.connect(sql_path)
        self.cur = self.conn.cursor()
        self.cur.execute(f"SELECT Symbol FROM {table_name} WHERE Symbol = '{self.symbol}'")
        symbol_exists = self.cur.fetchone()
        if symbol_exists is None :
            raise ValueError(f"Symbol '{self.symbol}' does not exist in the '{table_name}' table in {sql_path}.")  
    ...
    def market(self) : 
        ...
        """
        soup = self.wiki_soup()
        if (soup == 'NA') or (soup is None) : return 'NA'
