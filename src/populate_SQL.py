...
latest_date_dict = {
    ...
    "month": 3,
    "day": 5,
}  # this is the latest date for which data will be stored.
historical_period = {
    "years": 5,
    "months": 0,
    "days": 0,
}  # duration of historical period for which to retrieve data.
"""
    ...
    try:
        ...
        if (
            ...
            parser.add_argument(
                ...
                default=sqlite_file_path,
            )
            parser.add_argument(
                "--wikipedia_url",
                help="URL of the Wikipedia page",
        ...
        print(
            f"SQLite error occurred: {sqlite_error}\n\n******* Program terminated *******\n"
        )
        sys.exit()

    except subprocess.CalledProcessError as process_error:
        print(
            f"An error occurred when trying to run {scraping_script}:\n{process_error.stderr}\n\n******* Program terminated *******\n"
        )
        sys.exit()
...
def create_sql_table_per_stock(symbols):
    ...
    for symbol in symbols:  # for each stock
        try:
            cur.execute(
                f"SELECT 1 FROM sqlite_master WHERE type='table' AND name='{symbol}'"
            )  # check if table already exists in SQLite file
    ...
    )  # number of tables in SQLite file
    ...
    print(
        f'SQL database contains a total of {num_tables} tables, including the "Stocks" table + historical data for {len(symbols)} stocks.\n'
    )
...
def period_to_retrieve(latest_date, historical_period):
    ...
    earliest_date = latest_date - relativedelta(
        years=historical_period["years"],
        months=historical_period["months"],
        days=historical_period["days"],
    )
    ...
    print(
        f"Historical information will be retrieved for the time period (YYYY-MM-DD): {earliest_date_str} - {latest_date_str}\n"
    )
...
def extract_data(data, earliest_date_str, latest_date_str):
    ...
    dates = [
        ...
        if (date >= earliest_date_str and date <= latest_date_str)
    ]
...
def calculate_from_data(tables_with_data):
    ...
    try:
        for table in tables_with_data:
            cur.execute(
                f"UPDATE '{table}' SET DailyChange = ROUND(100*Close/Open - 100, {digits_after_decimal})"
            )
            cur.execute(
                f"UPDATE '{table}' SET DailyRange = ROUND(High - Low, {digits_after_decimal})"
            )
    except Exception as e:
...
def main(key):
    ...
    symbols = get_stock_list_from_sql(
        scraping_script
    )  # get symbols of stocks for which to get historical data from existing SQLite table produced by the scraping script
    create_sql_table_per_stock(
        symbols
    )  # creates an individual table (if does not yet exist) for each stock, for storing its historical data

    latest_date = datetime(
        latest_date_dict["year"], latest_date_dict["month"], latest_date_dict["day"]
    )  # timestamp of latest date for which data will be retrieved
    earliest_date_str, latest_date_str = period_to_retrieve(
    ...
    if len(stock_tables_with_data) == len(
        symbols
    ):  # has data already been retrieved for all stocks in S&P 500 index?
